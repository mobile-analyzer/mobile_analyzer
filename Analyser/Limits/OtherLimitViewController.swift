import UIKit
import Firebase
/// Страница с лимитом на прочее
class OtherLimitViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ref.child("users").child(uid!).child("otherLimit").observe(.value) { (snap) in
            if let value = snap.value as? [String:AnyObject]{
                let progress = value["progress"] as! Double
                let limit = value["limit"] as! Double
                if progress > limit{
                    let alert = UIAlertController(title: nil, message: "Exceeded the limit on the category of other!", preferredStyle: .alert)
    ...
    @objc func addButtonClicked(_ : UIButton) {
        ...
        let action = UIAlertAction(title: "Set", style: .default) { (action) in
            ...
            self.ref.child("users").child(self.uid!).child("otherLimit").setValue(["flag": "true", "limit": limit!, "progress": 0])
    ...
    @objc func deleteButtonClicked(_: UIButton){
        ...
        let deleteAction = UIAlertAction(title: "Delete", style: .default) { (action) in
            ...
            self.forAddButton()
            self.ref.child("users").child(self.uid!).child("otherLimit").setValue(["flag": "false", "limit": 0, "progress": 0])
